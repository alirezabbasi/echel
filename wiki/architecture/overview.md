...
| ID | Model | Rationale | Source IDs | Operational Boundary | ADR Coverage | Status |
| --- | --- | --- | --- | --- | --- | --- |
| ARCH-050 | Local-first repository and cockpit runtime | Echel must preserve product memory in committed files and keep lifecycle commands inspectable before any hosted runtime is justified. | REQ-006, NFR-001, NFR-005 | Product repository, local CLI, local cockpit, local provider configuration | ADR-0004 | Accepted |
| ARCH-051 | Hosted or distributed runtime deferred | Team collaboration, tenancy, permissions, public APIs, or remote orchestration require explicit product requirements and a future ADR. | Future requirement | Outside current MVP architecture | Future ADR required | Planned |
