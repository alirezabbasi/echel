...
Call `preview_write(record, expectation)` to obtain the destination, record ID,
content digest, replacement state, and whether bytes would change. Preview
...
Every preview and write requires an explicit [revision precondition](optimistic-concurrency.md).
Identical content is a no-op. The
